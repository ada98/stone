...
import stone.chap9.ClassInfo;

import java.util.ArrayList;

/**
 * @Author ada
 * @Date 2022/6/7 11:22 PM
 * @Version 1.0
 */
public class OptClassInfo extends ClassInfo {
    ...
    public void copyTo(Symbols f, Symbols m, ArrayList<DefStmnt> mlist) {
        f.append(fields);
        m.append(methods);
        for (DefStmnt methodDef : methodDefs) {
            mlist.add(methodDef);
        }
    }
    ...
    public Object method(OptStoneObject self, int index) {
        ...
        return new OptMethod(def.parameters(), def.body(), environment(), ((ObjOptimizer.DefStmntEx2) def).locals(), self);
    }
